...
Utility/
└── extract_wilab_db_to_mat.m                          % Convert WiLabVISim SQLite output to .mat
README.md
```
...
1. **Prepare your scenario data** — Run WiLabVISim and export the SQLite database,
   then convert it to a `.mat` file using the provided utility:
   ```matlab
   extract_wilab_db_to_mat('<path-to-your-wilab-run-folder>')
   ```
   This will generate a `wilab_export.mat` file. Point `cfg.inputMat` in `main_sim.m`
   to this file before running.

2. **Set your working directory** — Open MATLAB and set the working directory
   to the ARIFOMA repo root.

3. **Run the simulation** — Choose the entry point that matches your sensor layout:
   - `main_sim.m`    — four corner-mounted radar sensors (FL, FR, RL, RR)
   - `main_sim_FF.m` — single front-facing radar sensor (FF)

## WiLabVISim Data Export

ARIFOMA reads pre-computed traffic and interference propagation scenarios
generated by [WiLabVISim](https://www.jku.at/en/institute-of-signal-processing/research/automotive-radar/). The utility `utils/extract_wilab_db_to_mat.m`
converts the WiLabVISim SQLite output into the `.mat` format expected by ARIFOMA.

**Requirements:** MATLAB Database Toolbox (`sqlite` function).

## Sensor Layout Support

| Entry point     | Sensor layout                        | cfg.FrontSensor |
|-----------------|--------------------------------------|-----------------|
| `main_sim.m`    | Four corners — FL, FR, RL, RR        | `false` (default) |
| `main_sim_FF.m` | Single front-facing sensor — FF      | `true`          |

## Output

Results are saved as `radarCtx<sceneTag>.mat` in the repo root and can be
fed directly into ARIFOMA post-processing scripts.
